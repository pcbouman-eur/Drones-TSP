...
/**
 * Unit tests for Solution, checking whether they can be written to a file
 * and then read back again. Based on solution generated by the RandomSolver.
 * @author Paul Bouman
 *
 */
public class SolutionIOTest
{
	...
	{
		long seed = 1234;
		for (GeometricInstance gi : TestInstances.getGeometricInstances(100))
		{
			RandomSolver<Vec2D> rs = new RandomSolver<>(seed);
			Solution<Vec2D> s = rs.solve(gi);
			runIOTest(s);
		}
		for (GraphInstance<Integer> gi : TestInstances.getGraphInstances(100))
		{
		...
		for (MatrixInstance<Vec2D> mi : TestInstances.getMatrixGeometricInstances(100))
		{
